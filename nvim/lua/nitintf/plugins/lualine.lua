...
local function diagnostics_component()
	...
	function diagnostics:update_status()
		...
		for name, hl in pairs(self.diagnostics.highlight_groups) do
			...
			backgrounds[name] = lualine_utils.extract_highlight_colors(
				colors[name]:match("%%#(.-)#"),
				"bg"
			)
		end
		...
		for _, section in ipairs(self.diagnostics.sections) do
			if diagnostics_results[section] ~= nil then
				padding = previous_section
						and (backgrounds[previous_section] ~= backgrounds[section])
						and " "
					or ""
				previous_section = section

				local icon = self.diagnostics.symbols[section]
				local buffer_total = diagnostics_results[section][1] ~= 0
						and diagnostics_results[section][1]
					or "-"
...
function M.setup()
	...
	local palette = require("nitintf.theme.palette")
	local lsp = require("nitintf.utils.lsp")
	...
	local theme = {
		...
		command = {
			a = { fg = color.inverted_text, bg = palette.yellow, gui = "bold" },
		},
		insert = {
			a = { fg = color.inverted_text, bg = palette.green, gui = "bold" },
		},
		visual = {
			a = { fg = color.inverted_text, bg = palette.purple, gui = "bold" },
		},
		terminal = {
			a = { fg = color.inverted_text, bg = palette.cyan, gui = "bold" },
		},
		replace = {
			a = { fg = color.inverted_text, bg = palette.red, gui = "bold" },
		},
		inactive = {
